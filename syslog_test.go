...
func TestSyslogWriterTCP(t *testing.T) {
	...
	for _, level := range []string{"trace", "debug", "info", "warning", "error", "fatal", "panic", "hahaha"} {
		...
		fmt.Fprintf(w, `{"time":"2019-07-10T05:35:54.277+08:00","level":"%s","caller":"test.go:42","error":"i am test error","foo":"bar","n":42,"message":"hello journal writer"}`+"\n", level)
		fmt.Fprintf(w, `{"ts":1234567890,"level":"%s","caller":"test.go:42","error":"i am test error","foo":"bar","n":42,"message":"hello journal writer"}`+"\n", level)
	}

	_, err := fmt.Fprint(w, `{"time":"2019-07-10T05:35:54.277Z","level":"error","msg":"a test message\n"}`+"\n")
	if err != nil {
		t.Errorf("write syslog writer error: %+v", err)
	}

	_, err = fmt.Fprint(w, "a long long long long message.\n")
	if err != nil {
		t.Errorf("write syslog writer error: %+v", err)
	}
	...
	_, err = fmt.Fprint(w, "a long long long long message again.\n")
	if err != nil {
		t.Errorf("write syslog writer error: %+v", err)
	}
}
...
func TestSyslogWriterTCPError(t *testing.T) {
	...
	w.Close()
	fmt.Fprint(w, "a long long long long message again.\n")
}
